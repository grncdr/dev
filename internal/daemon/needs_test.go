...
func TestDisabledProcessNeverStarts(t *testing.T) {
	baseDir := t.TempDir()
	repoDir := filepath.Join(baseDir, "repo")
	if err := os.MkdirAll(repoDir, 0o755); err != nil {
		t.Fatal(err)
...
`
	...
	m := NewManager()
	slug := defaultSlugForRepo(t, repoDir)
	status, err := m.StartProcessesFromDir(slug, repoDir, []string{"db"}, false)
	if err != nil {
		t.Fatalf("start disabled process: %v", err)
	}
	if len(status.Processes) != 0 {
		t.Fatalf("expected disabled process to be skipped, got %+v", status.Processes)
	}
	_, _ = m.StopWorktreeFromDir(slug, repoDir)
}

func TestProcessNeedsIgnoresDisabledDependencies(t *testing.T) {
	baseDir := t.TempDir()
	repoDir := filepath.Join(baseDir, "repo")
	if err := os.MkdirAll(repoDir, 0o755); err != nil {
		t.Fatal(err)
...
[process.api]
port = 18080
command = "sh -c \"echo ${DEV_PORT_DB},${PORT} > dep_ports.txt; trap 'exit 0' INT TERM; while :; do sleep 1; done\""
needs = ["db"]
`
	if err := os.WriteFile(filepath.Join(repoDir, ".dev.toml"), []byte(cfg), 0o600); err != nil {
		t.Fatal(err)
	...
	if !seen["api"] {
		t.Fatalf("expected api to start, got %+v", status.Processes)
	}
	if seen["db"] {
		t.Fatalf("did not expect disabled dependency to start, got %+v", status.Processes)
	}
	...
	for {
		...
		if readErr == nil {
			got := strings.TrimSpace(string(data))
			if got != ",18080" {
				t.Fatalf("expected disabled dependency to be ignored, got %q", got)
			}
			break
		}
		select {
